...
import { InfoDto, MatchListDto } from '../interfaces/match.interface';
...
export class RiotService {
  ...
  async getMatchDataByMatchId(matchId: string): Promise<InfoDto> {
  ...
  async getMatchListDataByPuuid(puuid: string): Promise<MatchListDto[]> {
    ...
    const matches: InfoDto[] = await Promise.all(matchListData);
